...
var initHttpServer = () => {
    ...
    app.post('/mineBlock', (req, res) => {
        // var newBlock = generateNextBlockWithEvenHash(req.body.data);
        var newBlock = generateBlockWithSomePrime(req.body.data)
        addBlock(newBlock);
...
function sumOfPrimesInHash(hash) {
    let sum = 0;
    for (let i = 0; i < hash.length; i++) {
        let num = parseInt(hash[i], 16);
        if (isPrime(num)) {
            sum += num;
        }
    }
...
function countLettersInHash(hash) {
    var letterCount = 0;

    for (var i = 0; i < hash.length; i++) {
        // Проверяем, является ли текущий символ буквой
        if (/[a-zA-Z]/.test(hash[i])) {
            letterCount++;
        }
    }
...
var generateNextBlockWithEvenHash = (blockData) => {
...
var generateBlockWithSomePrime = (blockData) => {
    ...
    } while (primeNumber !== countLettersInHash(nextHash));
